...
function getInfoWindowTemplate(obj) {
    ...
    templateObj.profilePicUrl = obj.profilePicUrl ? obj.profilePicUrl : 'img/defaults/profile.png';
    ...
    for (var prop in templateObj) {
        ...
        mainTemplate = mainTemplate.replace(reg, templateObj[prop])
    }
...
roadFravel.controller('SearchCtrl', ["$scope", "rf_fetchResults", "toastr", "$timeout", function($scope, rf_fetchResults, toastr, $timeout) {
    ...
    function sliderEnd(sliderId, modelValue, highValue) {
        ...
        $scope.applyFilter({
            type: 'source',
            value: {
                lat: $scope.locations.source.latLng.lat,
                lng: $scope.locations.source.latLng.lng,
    ...
    function fetchResults() {
        ...
        var promise = rf_fetchResults.fetchPool({
            ...
            filtersReqd: rf_fetchResults.aggregationsRequired,
            postQueryFilter: recurtype ? [{
                value: recurtype,
                type: "recurtype"
            }] : undefined,
    ...
    function fetchResultsWithSource(){
    	$scope.applyFilter([{
            type: 'source',
            value: {
                lat: $scope.locations.source.latLng.lat,
...
roadFravel.controller('MyPoolsCtrl', ["$scope", "rf_fetchMyPools", function($scope, rf_fetchMyPools) {
    ...
    function fetchMyPools(inp) {
        var prom = rf_fetchMyPools.fetchPool({
            page: inp.page,
